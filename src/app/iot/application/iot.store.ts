...
/**
 * Reactive store for managing the state of the Iot Bounded Context.
 */
@Injectable({
  providedIn: 'root',
})
export class IotStore {
  private readonly repository = inject(IotRepository);
  ...
  private readonly latestSnapshotSignal = signal<IotSnapshot | null>(null);
  private readonly historySignal = signal<IotSnapshot[]>([]);
  ...
  readonly vehicleDevices = computed<VehicleIot[]>(() => {
  ...
  loadInitialData(): void {
    ...
    forkJoin({
      ...
      error: () => {
        this.loadingSignal.set(false);
        this.errorSignal.set('Failed to load initial iot data');
      }
    });
  }
  ...
  private loadIotData(device: ObdDevice): void {
    this.loadingSignal.set(true);
    const to = new Date();
